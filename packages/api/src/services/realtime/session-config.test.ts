...
describe('buildSessionConfig', () => {
  ...
  it('force require_approval=never sur tous les MCP', () => {
    const cfg = buildSessionConfig({ ...baseInput, tools: { bible: true, webSearch: false } });
  ...
  it('filtre MCP selon toggles (bible off)', () => {
    const cfg = buildSessionConfig({ ...baseInput, tools: { bible: false, webSearch: false } });
  ...
  it('ajoute web_search si toggle on', () => {
    const cfg = buildSessionConfig({ ...baseInput, tools: { bible: false, webSearch: true } });
    expect(cfg.session.tools!.some((t) => t.type === 'web_search')).toBe(true);
  });

  it('inclut write_to_chat function tool toujours', () => {
    const cfg = buildSessionConfig({ ...baseInput, mcpTools: [], tools: { bible: false, webSearch: false } });
